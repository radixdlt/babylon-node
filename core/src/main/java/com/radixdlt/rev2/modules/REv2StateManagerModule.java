    ...
    switch (databaseType) {
      case ROCKS_DB -> install(
          new AbstractModule() {
            @Provides
            @Singleton
            DatabaseTypeConfig databaseConfig(@NodeStorageLocation String nodeStorageLocation) {
              return DatabaseTypeConfig.rocksDB(
                  new File(nodeStorageLocation, "rocks_db").getPath(), databaseConfig);
            }
          });
      case IN_MEMORY -> install(
          new AbstractModule() {
            ...
            protected void configure() {
              bind(DatabaseTypeConfig.class)
                  .toInstance(DatabaseTypeConfig.inMemory(databaseConfig));
            }
          });
    }
