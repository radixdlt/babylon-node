...
public final class REV2GenesisFromOlympiaTest {
  ...
  public void genesis_from_olympia_should_initialize_correct_xrd_balances() throws IOException {
    ...
    final var genesisData = OlympiaStateToBabylonGenesisMapper.toGenesisData(olympiaEndState);
    try (var test = createTest(genesisData)) {
      test.startAllNodes();
      var stateReader = test.getInstance(0, REv2StateReader.class);
      final var xrdIndex = OlympiaStateToBabylonGenesisMapper.findXrdResourceIndex(olympiaEndState);
