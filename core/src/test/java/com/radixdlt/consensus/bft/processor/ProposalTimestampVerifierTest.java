...
public class ProposalTimestampVerifierTest {
  ...
  public void when_process_proposal_with_decreasing_timestamp_then_should_be_rejected() {
    ...
    verify(proposalRejectedDispatcher, times(1)).dispatch(any());
  }

  @Test
  public void when_process_proposal_with_timestamp_too_far_in_the_future_then_should_be_rejected() {
    final var currentSystemTime = System.currentTimeMillis();
    ...
    sut.processProposal(
        createProposalWithTimestamps(
            currentSystemTime - 1,
            currentSystemTime + MAX_ACCEPTABLE_PROPOSAL_TIMESTAMP_RUSH_MS + 1));
    verify(forwardTo, never()).processProposal(any());
    verify(proposalRejectedDispatcher, times(1)).dispatch(any());
  }
  ...
  private Proposal createProposalWithTimestamps(long prevTimestamp, long currentTimestamp) {
    ...
    final var vertex = vertexWithProposerTimestamps(prevTimestamp, currentTimestamp);
