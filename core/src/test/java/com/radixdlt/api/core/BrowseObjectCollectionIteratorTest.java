...
public final class BrowseObjectCollectionIteratorTest extends DeterministicCoreApiTestBase {

  private static final int SMALL_PAGE_SIZE = 3;

  @Test
  public void browse_api_object_collection_iterator_pages_through_all_entries() throws Exception {
    try (var test = buildRunningServerTest()) {
      ...
      // fetch all keys from resource package's blueprint definitions
      final var request =
          new BrowseObjectCollectionIteratorRequest()
              .entityAddress(wellKnownAddresses.getResourcePackage())
              .network(networkLogicalName)
              .collectionName("blueprint_version_definition");
      final var allKeys = getBrowseApi().browseObjectCollectionIteratorPost(request).getPage();

      // make sure that we should expect some continuation tokens when paging
      assertThat(allKeys.size()).isGreaterThan(SMALL_PAGE_SIZE);

      // now fetch all the same keys, but in small pages
      final List<CollectionEntryKey> pagedKeys = new ArrayList<>();
      @Nullable String continuationToken = null;

      while (true) {
        final var smallResponse =
            getBrowseApi()
                .browseObjectCollectionIteratorPost(
