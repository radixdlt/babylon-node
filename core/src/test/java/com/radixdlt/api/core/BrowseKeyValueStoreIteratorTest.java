...
public final class BrowseKeyValueStoreIteratorTest extends DeterministicCoreApiTestBase {
  ...
  public void browse_api_kv_store_iterator_pages_through_all_entries() throws Exception {
    try (var test = buildRunningServerTest()) {
      ...
      // locate an internal KV-Store owned by Faucet (where each transaction creates an entry):
      ...
      final var transactionsKeyValueStoreAddress =
          faucetFields.stream()
              .map(field -> (Map<?, ?>) field)
      ...
      // execute a couple of dummy transactions (just to create multiple entries in that KV-Store):
      ...
      final var request =
          new BrowseKeyValueStoreIteratorRequest()
              .entityAddress(transactionsKeyValueStoreAddress)
              .network(networkLogicalName);
