...
pub async fn create_server<F>(
    bind_addr: &str,
    shutdown_signal: F,
    browse_api_state: BrowseApiState,
    ...
    F: Future<Output = ()>,
{
    let router = Router::new()
        ...
        .with_state(browse_api_state);

    let metrics = Arc::new(BrowseApiMetrics::new(metric_registry));

    let prefixed_router = Router::new()
        .nest("/browse", router)
        .route("/", get(handle_missing_browse_path))
...
pub struct BrowseApiServerConfig {
