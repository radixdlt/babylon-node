...
use log::warn;
// TODO some of those opentelemetry-related imports are not necessary
use opentelemetry::{
    global,
    ...
    C: Has<XSpanIdString>  + Send + Sync + 'static
{
        ...
        C: Has<XSpanIdString>  + Send + Sync + 'static
    {
        ...
        // TODO is this autogenerated code? then something like this must be added by generator
        // TODO what's more it is believed hyper has built-in support for tracing
        let parent_otel_context = global::get_text_map_propagator(|propagator| {
            propagator.extract(&HeaderExtractor(&headers))
        });
        let mut otel_span = global::tracer("example/server").start_with_context("hello", &parent_otel_context);

        otel_span.add_event("handling this...", Vec::new());
