...
use radix_engine_interface::types::{NonFungibleResourceManagerOffset, OBJECT_BASE_MODULE};
...
pub(crate) async fn handle_state_non_fungible(
    ...
    if non_fungible_local_id.id_type() != id_type_substate {
        return Err(ExtractionError::WrongNonFungibleIdType {
            expected: id_type_substate,
            actual: non_fungible_local_id.id_type(),
        }
        .into_response_error("non_fungible_id"));
    }

    let _non_fungible_substate_key = SubstateKey::Map(non_fungible_local_id.to_key());
