...
use radix_engine_queries::typed_substate_layout::{TypedSubstateKey, TypedMainModuleSubstateKey};
use std::ops::Deref;
...
pub(crate) async fn handle_state_non_fungible(
    ...
    Ok(StateNonFungibleResponse {
        non_fungible: Some(to_api_non_fungible_resource_manager_data_substate(
            &mapping_context,
            &TypedSubstateKey::MainModule(TypedMainModuleSubstateKey::NonFungibleResourceData(non_fungible_id)),
            &substate,
        )?),
    })
    .map(Json)
