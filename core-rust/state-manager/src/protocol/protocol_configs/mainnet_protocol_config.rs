...
pub fn mainnet_protocol_config() -> ProtocolConfig {
    const EAGLE_RAY_ENACTMENT_EPOCH: u64 = 339898;

    // See config_printer.rs > print_calculated_protocol_config_code()
    ProtocolConfig::new_with_triggers_and_moratoriums(
        hashmap! {
            ...
            ProtocolVersionName::eagle_ray() => EnactAtStartOfEpochUnconditionally(
                Epoch::of(EAGLE_RAY_ENACTMENT_EPOCH),
            ),
        },
        [
            // Mainnet incident of 1 September 2026.
            UserTransactionMoratorium {
                from_inclusive: Epoch::of(339897),
                to_exclusive: Epoch::of(EAGLE_RAY_ENACTMENT_EPOCH),
            },
        ],
    )
}
