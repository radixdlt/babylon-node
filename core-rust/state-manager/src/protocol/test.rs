...
fn prepare_and_commit_round_update(state_manager: StateManager) -> (PrepareResult, CommitSummary) {
    let read_db = state_manager.database.read_current();
    let last_proof: LedgerProof = read_db.get_last_proof().unwrap();
    let last_epoch_proof: LedgerProof = read_db.get_last_epoch_proof().unwrap();
    ...
    let proposer_address = last_epoch_proof
        .ledger_header
        .next_epoch
        .unwrap()
        .validator_set
