...
mod tests {
    ...
    use radix_engine::wasm::{DefaultWasmEngine, InstructionCostRules, WasmInstrumenter, WasmMeteringParams};
    use scrypto::engine::types::{GlobalAddress, RENodeId, SubstateId};
    ...
    fn test_epoch_update() {
        let mut store = StateManagerDatabase::from_config(DatabaseConfig::InMemory);

        let epoch = store.get_epoch();
        ...
        let mut transaction_executor =
            TransactionExecutor::new(&mut store, &mut scrypto_interpreter);
