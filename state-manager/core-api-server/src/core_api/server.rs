...
use state_manager::StateManagerImpl;

use state_manager::mempool::Mempool;

pub async fn create<M : 'static + Mempool + Send + Sync , S : 'static + Send + Sync , F>(
    bind_addr: &str,
    shutdown_signal: F,
    state_manager: Arc<Mutex<StateManagerImpl<M, S>>>,
) where
    F: Future<Output = ()>,
{
...
pub struct Server<M : 'static + Mempool + Send + Sync , S: 'static + Send + Sync , C> {
    state_manager: Arc<Mutex<StateManagerImpl<M, S>>>,
    marker: PhantomData<C>,
}

impl<M: 'static + Mempool + Send + Sync, S: 'static + Send + Sync, C> Clone for Server<M, S, C> {
    fn clone(&self) -> Self {
        Self {
...
impl<M : Mempool + 'static + Send + Sync , S : 'static + Send + Sync , C> Server<M, S, C> {
    pub fn new(state_manager: Arc<Mutex<StateManagerImpl<M, S>>>) -> Self {
        Server {
            state_manager,
            marker: PhantomData,
        }
...
#[async_trait]
impl<M : 'static + Mempool + Send + Sync , S : 'static + Send + Sync , C> Api<C> for Server<M, S, C>
where
