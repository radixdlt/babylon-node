...
public record CustomTypeKnownLengthCodec<T>(
    TypeId typeId,
    int knownLength,
    Function<T, byte[]> encodeToBytes,
    Function<byte[], T> decodeFromBytes)
    implements Codec<T> {
  ...
  public void encodeWithoutTypeId(EncoderApi encoder, T obj) {
    ...
    if (bytes.length != knownLength) {
      throw new SborEncodeException(
          String.format(
              "Custom payload on encode of length %s but expected %s", bytes.length, knownLength));
    }
  ...
  public T decodeWithoutTypeId(DecoderApi decoder) {
    final var bytes = decoder.readBytes(knownLength);
