...
import static com.radixdlt.sbor.codec.constants.TypeId.TYPE_I8;
...
public final class ByteCodec implements Codec<Byte> {
  ...
  /**
   * Note that shorts are always signed in Java. If signed is false, you may have a positive SBOR
   * value mapping to a negative Java value - but this is pretty normal/expected with Java bytes, so
   * we don't add the possibility for asserts (unlike the other types). This also improves
   * performance marginally on what might be a critical code path.
   *
