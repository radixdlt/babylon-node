...
public interface MapCodec2 {
    record MapCodecViaHashMap<TKey, TItem>(
            TypeId mapTypeId,
            Codec<TKey> keyCodec,
            Codec<TItem> itemCodec,
            Functions.Func1<Map, Integer> getSize,
            Functions.Func1<Map<TKey, TItem>, Iterable<Map.Entry<TKey, TItem>>> getIterable,
            Functions.Func1<HashMap<TKey, TItem>, Map> mapFromHashMap)
        ...
        public Map<TKey, TItem> decodeTo(DecoderApi decoder, Function<Integer, Map<TKey, TItem>> supplier) {
           // decoder.expectType(keyCodec.getTypeId());
           // decoder.expectType(itemCodec.getTypeId());

            var length = decoder.readInt();
            var map = supplier.apply(length);
        ...
        @Override
        public void encodeWithoutTypeId(EncoderApi encoder, Map map) {
            encodeFromIterable(encoder, getSize.apply(map), getIterable.apply(map));
        }

        @Override
        public Map<TKey, TItem> decodeWithoutTypeId(DecoderApi decoder) {
     ...
     * @return
     */
    static <TKey, TItem> MapCodecViaHashMap<TKey, TItem> forMap(
            Codec<TKey> keyCodec, Codec<TItem> itemCodec, TypeId mapTypeId) {
        mapTypeId.assertMapType();
        return new MapCodec2.MapCodecViaHashMap<>(
