...
public abstract class BasicDefaultSbor {
  private static final Sbor sbor = new BasicSbor(CodecMap.DEFAULT_RESOLVER);

  public static <T> byte[] encode(T value) {
    return sbor.encode_payload(value);
  }

  public static <T> byte[] encode(T value, Class<T> clazz) {
    return sbor.encode_payload(value, clazz);
  }

  public static <T> byte[] encode(T value, TypeToken<T> type) {
    return sbor.encode_payload(value, type);
  }
  ...
  public static <T> T decode(byte[] input, Class<T> clazz) {
    return sbor.decode_payload(input, clazz);
  }

  public static <T> T decode(byte[] input, TypeToken<T> type) {
    return sbor.decode_payload(input, type);
  }

  public static <T> T decode(byte[] input, Codec<T> codec) {
    return sbor.decode_payload(input, codec);
  }
}
