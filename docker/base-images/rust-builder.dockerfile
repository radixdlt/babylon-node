...
# We do a pre-emptive build so that the builder layer contains pre-built dependencies,
# to speed up future compilation (assuming most dependencies are the same in the builder image and the built code).
RUN cd core-rust && \
