...
|---|---|---|---|
| `ci.yml` | Every PR; push to `develop`, `main`, `release/*` | Unit/integration tests, Sonar, Snyk (deps+code), Codecov, cross-compile check | [Runs](https://github.com/radixdlt/babylon-node/actions/workflows/ci.yml) |
...
|---|---|---|---|---|---|
| `ci.yml` | `AWS_ROLE_NAME_SNYK_SECRET`; `AWS_SECRET_NAME_SNYK`; `COMMON_SECRETS_ROLE_ARN`; `AWS_SECRET_NAME_CODECOV`; `GITHUB_TOKEN` | Snyk jobs fetch via **external** `public-iac-resuable-artifacts/fetch-secrets` using `AWS_ROLE_NAME_SNYK_SECRET`/`AWS_SECRET_NAME_SNYK`; `build` job fetches Codecov secret the same way using `COMMON_SECRETS_ROLE_ARN`/`AWS_SECRET_NAME_CODECOV`, and fetches the Sonar token via the **local** `fetch-secrets` action from the literal path `github-actions/common/sonar-token` | `selfhosted-ubuntu-22.04-16-cores` (build/sonar, steadystate-integration, targeted-integration) | none | Snyk (deps/license + code scan), SonarCloud/SonarQube, Codecov.io |
